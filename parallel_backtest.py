...
def process_grid(date,grid, last_price, price, positions, order_size, required_margin_rate, required_margin, margin_maintenance_rate, order_margin,order_capacity_flag,margin_maintenance_flag, lock):
    if min(last_price,price) <= grid <= max(last_price,price):
         ...
         if margin_maintenance_rate > 100 and order_capacity > 0:		#you dont need to confirm that order_capacity is more than 0 or not because order_margin and required_margin is equal so order_capacity is more than 0 absolutely
            ...
            print(f"Opened Buy position at {price} with grid {grid}")
...
def process_grids_parallel(positions,order_margin,required_margin, date,grids, last_price, price, order_size, required_margin_rate, margin_maintenance_rate,order_capacity_flag,margin_maintenance_flag, lock):
    with Pool() as pool:
        for grid in grids:
            pool.apply_async(process_grid, args=(date,grid, last_price, price, positions, order_size, required_margin_rate, required_margin, margin_maintenance_rate, order_margin,order_capacity_flag,margin_maintenance_flag,lock))
...
def process_position(margin_maintenance_flag, effective_margin, margin_deposit, realized_profit, required_margin, pos, price, profit_width, order_size, required_margin_rate,lock):
    ...
    if pos[2] == 'Buy' and pos[1] <= len(data) - 1:
        ...
        if price - pos[3] >= profit_width:
            ...
            pos[7] += profit
            subtract_required_margin = pos[5]
            required_margin -= subtract_required_margin
            pos[5] = 0
            pos[2] = 'Buy-Closed'
            #trades.append((pos[1], price, 'Sell'))
...
# 並列処理を使ったPosition closure processing
def process_positions_parallel(positions,effective_margin,margin_deposit, realized_profit, required_margin, margin_maintenance_flag, price, profit_width, order_size, required_margin_rate,lock):
    with Pool(processes=4) as pool:  # プロセス数は適宜調整
        for pos in positions:
            pool.apply_async(process_position, args=(margin_maintenance_flag, effective_margin, margin_deposit, realized_profit, required_margin, pos, price, profit_width, order_size, required_margin_rate,lock))
        pool.close()
        pool.join()
...
def process_swap_parallel(positions,date,num_positions,effective_margin,margin_maintenance_flag,lock):
    with Pool() as pool:
        # 各ポジションのスワップ値を並列に計算
        for pos in positions:
            pool.apply_async(process_swap, args=(pos,date,effective_margin,required_margin,num_positions,lock))
        pool.close()
        pool.join()
...
def process_losscut_parallel(margin_maintenance_flag, pos, positions, price, effective_margin, margin_deposit, realized_profit, required_margin,lock):
    with Pool() as pool:
        for pos in positions:
            pool.apply_async(process_losscut, args=(margin_maintenance_flag, pos, price, effective_margin, margin_deposit, realized_profit, required_margin,lock))
        pool.close()
        pool.join()
...
def traripi_backtest(calculator, data, initial_funds, grid_start, grid_end, num_traps, profit_width, order_size, entry_interval=None, total_threshold=None,strategy='standard', density=1):
    ...
    if strategy == 'long_only':
        ...
        with Manager() as manager:
            ...
            positions = manager.list()  # 共有リスト
            order_margin = manager.Value('d', sum(order_size * grid * required_margin_rate for grid in grids))  # 共有の浮動小数点変数
            ...
            for i in range(len(data)):
                ...
                if last_price is not None:
                    ...
                    if price != last_price:
                        order_margin.value, required_margin.value, positions, order_capacity_flag, margin_maintenance_flag.value = process_grids_parallel(positions, date,grids, last_price, price, order_size, required_margin_rate, effective_margin.value, required_margin.value, margin_maintenance_rate, order_margin.value,lock)
                ...
                positions, effective_margin.value, margin_deposit.value, realized_profit.value, required_margin.value, margin_maintenance_flag.value, price, profit_width, order_size, required_margin_rate,lock
                )

                if abs(required_margin) > 0:
                    margin_maintenance_rate = effective_margin / required_margin * 100
                    if margin_maintenance_rate <= 100:
                        margin_maintenance_flag = True  # マージンメンテナンスフラグを有効に

                else:
                    margin_maintenance_rate = np.float('inf')
                ...
                # 強制ロスカットのチェック
                effective_margin.value, margin_deposit.value, realized_profit.value, required_margin.value , margin_maintenance_flag.value = process_losscut_parallel(margin_maintenance_flag.value, pos, positions, price, effective_margin.value, margin_deposit.value, realized_profit.value, required_margin.value,lock)
                ...
                #check swap
                num_positions = manager.Value('d', 0)
                effective_margin.value, num_positions.value, margin_maintenance_flag.value = process_swap_parallel(positions,date,num_positions.value,effective_margin.value,num_positions.value,lock)
                ...
                if num_positions != 0:
                    RETURN.append(sum((pos[7]+pos[8]) for pos in positions)/num_positions.value)
...
#
    ...
    return effective_margin.value, margin_deposit.value, realized_profit.value, position_value, swap_value, required_margin.value, margin_maintenance_rate, entry_interval, total_threshold, sharp_ratio, max_draw_down
