...
def process_grids_parallel(manager, date, grids, last_price, price, effective_margin, order_size, i, required_margin_rate, margin_maintenance_rate,margin_maintenance_flag):
    # ロックオブジェクトを作成
    lock = manager.Lock()
    ...
    if pos[2] == 'Buy' and pos[1] <= len(data) - 1:
        ...
        if price - pos[3] >= profit_width:
            ...
            with lock:
                ...
                pos[5] = 0
                pos[2] = 'Buy-Closed'
            print(f"利益 {profit} で Buy ポジションをクローズしました。グリッド {pos[3]}, 有効証拠金: {effective_margin.value}, 必要証拠金: {required_margin.value} added_effective_margin: {add_effective_margin}")
...
def process_losscut(margin_maintenance_flag, pos, price, effective_margin, margin_deposit, realized_profit, required_margin, order_size, lock):
...
def process_losscut_parallel(manager, margin_maintenance_flag, positions, price, effective_margin, margin_deposit, realized_profit, required_margin, order_size):
    ...
    with Pool() as pool:
        pool.starmap(process_losscut, [(margin_maintenance_flag, pos, price, effective_margin, margin_deposit, realized_profit, required_margin, order_size, lock) for pos in positions])
                ...
                if price != last_price:
                    ...
                    if margin_maintenance_rate > 100 and order_capacity > 0:
                        order_margin, required_margin, positions, order_capacity_flag, margin_maintenance_flag = process_grids_parallel(
                            manager, date, grids, last_price, price, effective_margin, order_size, i, required_margin_rate, margin_maintenance_rate, margin_maintenance_flag
                        )
                        margin_maintenance_flag, margin_maintenance_rate = update_margin_maintenance_rate(effective_margin, required_margin)
                        if margin_maintenance_flag:
